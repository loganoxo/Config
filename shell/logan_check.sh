...
for repo in "${repos[@]}"; do
    if_clean=1
    if_has_remote=1
    echo -e "\033[31mChecking $repo \033[0m......"
    cd "$repo" || {
    ...
    if [ "$untracked" -gt 0 ]; then
        if_clean=0
        echo -e "    \033[31m UnTracked files:\033[0m $untracked"
    fi

    modified=$(git status --porcelain | grep -c '^[[:space:]]*[MADRC]') || true
    if [ "$modified" -gt 0 ]; then
        if_clean=0
        echo -e "    \033[31m UnCommited files:\033[0m $modified"
    fi
    ...
    if [ "$if_has_remote" -eq 1 ]; then
        ...
        if [ "$ahead" -gt 0 ]; then
            if_clean=0
            echo -e "    \033[31m UnPushed commits:\033[0m $ahead"
        fi
        ...
        if [ "$behind" -gt 0 ]; then
            if_clean=0
            echo -e "    \033[31m Local branch is behind upstream by\033[0m $behind\033[31m commits\033[0m"
        fi
    fi

    if [ "$if_clean" -eq 1 ]; then
        echo -e "    \033[31m everything is clean \033[0m"
    fi
    echo ""
    echo ""
done
