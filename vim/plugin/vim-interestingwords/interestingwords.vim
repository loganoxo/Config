...
if !exists('g:MapInterestingWords')
  let g:MapInterestingWords = '<leader>pz'
end
if !exists('g:MapUncolorAllWords')
  let g:MapUncolorAllWords = '<leader>pZ'
end

if g:interestingWordsDefaultMappings && !hasmapto('<Plug>InterestingWords')
    if g:MapInterestingWords != ''
        execute 'nnoremap <silent> '.g:MapInterestingWords.' :call InterestingWords("n")<cr>'
        execute 'vnoremap <silent> '.g:MapUncolorAllWords.' :call InterestingWords("v")<cr>'
    end
    if g:MapUncolorAllWords != ''
        execute 'nnoremap <silent> '.g:MapUncolorAllWords.' :call UncolorAllWords()<cr>'
    end
"    可以取消注释,启用n/N 上个/下个 的功能,我只用高亮功能
