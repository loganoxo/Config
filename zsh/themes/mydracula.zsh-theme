...
DRACULA_DISPLAY_TIME_LEFT=${DRACULA_DISPLAY_TIME_LEFT:-0}
DRACULA_DISPLAY_TIME_RIGHT=${DRACULA_DISPLAY_TIME_RIGHT:-1}
...
# Set to 1 to show full path of current working directory
DRACULA_DISPLAY_EXITCODE=${DRACULA_DISPLAY_EXITCODE:-1}

DRACULA_DISPLAY_DURATION=${DRACULA_DISPLAY_DURATION:-1}

# 在右边显示上次命令的返回码 {{{
#display_exitcode_func() {
#    local dracula_exitcode=$?
#    DISPLAY_EXITCODE=""
#    if ((DRACULA_DISPLAY_EXITCODE)); then
#        ((dracula_exitcode != 0)) && DISPLAY_EXITCODE="✘ $dracula_exitcode"
#    fi
#}
#add-zsh-hook precmd display_exitcode_func
#RPROMPT+='%F{red}%B$DISPLAY_EXITCODE%f'
if ((DRACULA_DISPLAY_EXITCODE)); then
    RPROMPT+='%F{red}%B%(?..✘ %?)%f '
fi
# }}}
...
dracula_time_start() {
    ...
    CMD_EXECUTED=true # 标记执行了命令
}

# 在命令执行后计算耗时并显示
dracula_time_end() {
    if [[ "$CMD_EXECUTED" != true ]]; then
        DRACULA_DISPLAY_DURATION_INFO=""
        return
    fi
    CMD_EXECUTED=false # 重置标记
    # 如果 `CMD_START_TIME` 存在，则计算耗时
    if [[ -n "$CMD_START_TIME" ]]; then
        local cmd_end_time=$EPOCHREALTIME
        local elapsed_time=$(printf "%.2f" "$(bc <<<"$cmd_end_time - $CMD_START_TIME")")
        DRACULA_DISPLAY_DURATION_INFO=$(
            if (($(bc <<<"$elapsed_time >= 2"))); then
                echo "%F{red}%B ${elapsed_time}s%f "
            elif (($(bc <<<"$elapsed_time >= 1"))); then
                echo "%F{yellow}%B ${elapsed_time}s%f "
            else
#                echo "%F{green}%B ${elapsed_time}s%f"
                echo ""
            fi
        )
    fi
...
if ((DRACULA_DISPLAY_DURATION)); then
    add-zsh-hook preexec dracula_time_start
    add-zsh-hook precmd dracula_time_end
    RPROMPT+='$DRACULA_DISPLAY_DURATION_INFO'
fi

# Time segment 右边 {{{
dracula_time_segment_right() {
    if ((DRACULA_DISPLAY_TIME_RIGHT)); then
        print -P "%D{$DRACULA_TIME_FORMAT} "
    fi
}

RPROMPT+=' %F{green}%B$(dracula_time_segment_right)'
# }}}
...
# Time segment 左边 {{{
dracula_time_segment_left() {
    if ((DRACULA_DISPLAY_TIME_LEFT)); then
        print -P "%D{$DRACULA_TIME_FORMAT} "
    fi
}

PROMPT+='%F{green}%B$(dracula_time_segment_left)'
# }}}
